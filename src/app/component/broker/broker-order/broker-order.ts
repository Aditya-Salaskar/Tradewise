...
executeOrder(orderId: string) {
  this.brokerOrderService.updateOrderStatus(orderId, 'EXECUTED').subscribe(() => {
...
declineOrder(orderId: string) {
  this.brokerOrderService.updateOrderStatus(orderId, 'DECLINED').subscribe(() => {
    alert(`Order ${orderId} declined successfully!`);
    this.ngOnInit(); // Refresh list
  });
}
  ...
  constructor(private router: Router, private brokerOrderService: BrokerOrderService) {}
